...
class PlaybackOverlayAndroidTest {
    ...
    @Test fun seekingToStartKeepsDateUntilPlaybackIsClosed() {
        ...
        val previous=prefs.getBoolean("street_detail",true)
        prefs.edit().putBoolean("street_detail",false).commit()
        try {
            val trip=com.traveler.feature.map.flat.CanyonDemo.trip()
        ...
        } finally { prefs.edit().putBoolean("street_detail",previous).commit() }
    }
