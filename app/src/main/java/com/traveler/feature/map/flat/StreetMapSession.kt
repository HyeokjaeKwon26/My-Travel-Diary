...
class StreetMapSession(context:Context,private val network:Boolean=false,
        ...
        while(isActive) {
            ...
            if(plan!=null) {
                for(tile in plan.tiles) {
                    ...
                    if(!active && synchronized(lock) { tile in bitmaps }) continue
    ...
    private fun load(tile:StreetTile,fetch:Boolean) {
        ...
        try {
            ...
            request.setRequestProperty("User-Agent","MyTravelDiary/${BuildConfig.VERSION_NAME} (+https://github.com/HyeokjaeKwon26/My-Travel-Diary)")
    ...
    fun paint(canvas:Canvas,w:MapFootprint,width:Int,height:Int,plan:StreetTilePlan) {
        synchronized(lock) {
            ...
            for(tile in plan.tiles) {
                ...
                var x=tile.x/n
                x+=round((w.left+w.right)/2-(x+.5/n))
                val left=((x-w.left)/(w.right-w.left)*width).toFloat()
                val top=((tile.y/n-w.top)/(w.bottom-w.top)*height).toFloat()
                val sideX=(width/(n*(w.right-w.left))).toFloat()
                val sideY=(height/(n*(w.bottom-w.top))).toFloat()
                ...
                canvas.drawBitmap(bitmap,src,RectF(left,top,left+sideX,top+sideY),paint)
            }
        }
    }
